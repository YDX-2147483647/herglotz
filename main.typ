...
4. $f$ 和 $cot$ 满足相同的#strong[递归]性质，或者说二倍角公式。
  ...
  #remark[要多想][
    如果你动手处理过 $sum 1/(n+1/2)$，可能已发现类似性质。
  ]
...
2. 连续
  ...
  - 在 $RR - ZZ$ 上，$h$ 自然连续。
  - 在 $ZZ$ 上，由于 $g$ 和 $csc$ 的留数相互抵消，$lim h = 0$。若补充定义 $h=0$，$h$ 即连续。

3. 递归 $and$ 连续 $=>$ 全零

   考虑第一个正周期 $[0, 1]$。由连续，$h$ 存在最大值点 $xi$ 及最大值 $m$。注意 $h$ 具有周期，因此 $m$ 也是全局最大值。

   由递归性质，$h(xi/2) + h(xi/2 + 1/2) = 2 h(xi) = 2m$——两点函数值的算术平均是最大值，那么两点必须都去最大值，于是 $h(xi/2) = m$。

   以此类推，$xi, xi/2, xi/2^2, xi/2^3, ...$ 处的 $h$ 都是 $m$。这一点列趋于 $0$，由连续，$h(0) = m$。

   然而按先前定义，$h(0) = 0$，于是只好 $m = 0$。
